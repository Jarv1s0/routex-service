...
const disableLinuxSandboxEnv = "ROUTEX_CORE_DISABLE_LINUX_SANDBOX"
...
func prepareLinuxSandboxRoot(launch *launchSession) (string, func() error, error) {
	...
	root, err := os.MkdirTemp("", "routex-core-sandbox-*")
	if err != nil {
...
func cleanupStaleLinuxSandboxRoots() {
	roots, err := filepath.Glob(filepath.Join(os.TempDir(), "routex-core-sandbox-*"))
	if err != nil {
