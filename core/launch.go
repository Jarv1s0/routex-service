...
func resolveCoreExecutablePath(rawPath string, allowEnv bool) (string, error) {
	...
	if path == "" && allowEnv {
		path = strings.TrimSpace(os.Getenv("ROUTEX_CORE_PATH"))
	}
	if path == "" {
...
func launchProfilePath() string {
	return filepath.Join(serviceConfigDir(), "routex", "core", "launch_profile.json")
}

func serviceConfigDir() string {
	if dir := os.Getenv("ROUTEX_CONFIG_DIR"); dir != "" {
		return dir
	}
